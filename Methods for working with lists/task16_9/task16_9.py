...
которые накопились у них друг к другу.
Однако выяснилось, что иногда один и тот же человек в разные дни выступал как в роли должника,
так и в роли кредитора.
...
Сначала вводится число N — количество друзей,
затем вводится число K — количество долговых расписок, после этого следует K троек чисел: номер друга,
взявшего в долг, номер друга, давшего деньги, и сумма. Гарантируется, что ни один друг не брал в долг сам у себя.
...
def calculate_debt(
    ...
    dict_of_debt[to_friend] += debt
    dict_of_debt[from_friend] -= debt
    return dict_of_debt
...
def main():
    ...
    for receipt in range(num_of_receipts):
        ...
        calculate_debt(dict_of_debt, to_friend, from_friend, debt)

    print("\nБаланс друзей:")
    for friend in dict_of_debt:
        debt = dict_of_debt.get(friend)
        print(f"{friend}: {debt}")
